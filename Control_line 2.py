...
class Control_lines_generator:
    ...
    def remove(self, targ):
        ...
        self.lib_all.pop(targ, bit_num)
        self.lib_fin.pop(targ, bit_num)
        point = 1
        for i in range(self.bit_len):
            if point & targ != 0 and self.lib_fin.contain(targ-point, bit_num-1):
                self.lib_all.pop(targ-point, bit_num-1)
                self.lib_fin.pop(targ-point, bit_num-1)
            point *= 2
